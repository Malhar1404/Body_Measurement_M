...
class Trainer:
    ...
    FEATURES:
    - CosineAnnealingLR scheduler (simple & reliable)
    - Wing Loss for better accuracy
    - Per-epoch LR stepping
    ...
    def __init__(self, config: Config, resume_from: str = None):
        ...
        initial_lr = config.training.INITIAL_LR if hasattr(config.training, 'INITIAL_LR') else 1e-4
        weight_decay = config.training.WEIGHT_DECAY if hasattr(config.training, 'WEIGHT_DECAY') else 1e-4
        ...
        print(f"✓ Optimizer: Adam (lr={initial_lr:.2e}, weight_decay={weight_decay:.2e})")
        
        # ===== CosineAnnealingLR Scheduler (Simple & Reliable) =====
        print("\n📈 Setting up CosineAnnealingLR scheduler...")
        
        max_epochs = config.training.MAX_EPOCHS if hasattr(config.training, 'MAX_EPOCHS') else 50
        ...
        print(f"✓ CosineAnnealingLR scheduler created:")
        print(f"  T_max: {max_epochs} epochs")
        print(f"  Initial LR: {initial_lr:.2e}")
        print(f"  Min LR: 1e-6")
        
        # Early stopper
        patience = config.training.EARLY_STOP_PATIENCE if hasattr(config.training, 'EARLY_STOP_PATIENCE') else 15
        min_delta = config.training.EARLY_STOP_MIN_DELTA if hasattr(config.training, 'EARLY_STOP_MIN_DELTA') else 0.001
        ...
        # Initialize loggers
        experiment_name = f"wing_cosine_{time.strftime('%Y%m%d_%H%M%S')}"
    ...
    def train(self, num_epochs):
        """Main training loop."""
        print("\n" + "="*80)
        print("TRAINING - HIP & BUST PREDICTION (Wing Loss + CosineAnnealingLR)")
        print("="*80)
